...
def FCN_Seg(self, is_training=True):
    ...
    with tf.variable_scope('First_conv'):
        conv1 = tc.layers.conv2d(self.tgt_image, 32, 3, 1, normalizer_fn=self.normalizer,
    ...
    if self.configuration == 1:
        ...
        # x.
        # Remember if dim(upsampled_features) > dim(input image) you must crop
        ...
        current_up5 = tf.nn.elu(current_up5)

        print('current_up5 shape', current_up5.get_shape()) # (?, 304, 304, 120)

        if current_up5.get_shape()[2] > self.tgt_image.get_shape()[2]:
            current_up5 = crop(current_up5, self.tgt_image)

        End_maps_decoder1 = slim.conv2d(current_up5, self.N_classes, [1, 1],
                                        scope='Final_decoder')  # (batchsize, width, height, N_classes)
    ...
    if self.configuration == 2:
        ...
        # End_maps_decoder1 = slim.conv2d(current_up3, self.N_classes, [1, 1],
        #                                scope='Final_decoder')  # (batchsize, width, height, N_classes)
    ...
    if self.configuration == 3:
        ...
        # End_maps_decoder1 = slim.conv2d(current_up4, self.N_classes, [1, 1],
        #                                scope='Final_decoder')  # (batchsize, width, height, N_classes)
